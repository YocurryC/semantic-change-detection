...
% assign the saliency value to each pixel     
     ...
     outname=[saldir imnames(ii).name(1:end-4) '_1stage1' '.png'];
...
% assign the saliency value to each pixel
    ...
    outname=[saldir imnames(ii).name(1:end-4) '_1stage2' '.png'];   
...
J(~isnan(J))=0;
J(isnan(J))=1;
outname=[foredir imnames(ii).name(1:end-4) '_foreground1' '.png'];   
imwrite(J,outname);
